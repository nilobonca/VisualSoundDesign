...
// Print first 15 matches with details
for (let i = 0; i < Math.min(matches.length, 15); i++) {
    const m = matches[i];
    console.log(`Match [${i}] - Step: ${m.step}, Tool: ${m.tool}`);
    if (m.tool === 'write_to_file') {
        console.log(`  TargetFile: ${m.args.TargetFile}`);
        console.log(`  Description: ${m.args.Description}`);
    } else if (m.tool === 'replace_file_content' || m.tool === 'multi_replace_file_content') {
        console.log(`  TargetFile: ${m.args.TargetFile}`);
        console.log(`  Description: ${m.args.Description}`);
    }
}
